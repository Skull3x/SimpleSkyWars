...
namespace SkygridRex\skywars\utils;
...
class Timer
{
	...
	// constanst time over dowm
	const SECONDS = 0;
	const MILLISECONDS = 1;
	const MICROSECONDS = 2;

	// number of microseconds in a second if you don't know (idk)
	...
	private static function _pushTime($cmd)
	{
		...
		if ($cmd == self::CMD_START) {
			// check if the timer has already been started (you never know)
